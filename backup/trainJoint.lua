...
-- GPU previous palette layers
local paletteUpdateInput = torch.CudaTensor()
    ...
    local feval = function(x)
        ...
        for superBatch = 1, opt.transformerSuperBatches do
            ...
            if superBatch == 1 then
                paletteUpdateInput:resize(model.transformerOutput.data.module.output:size()):copy(model.transformerOutput.data.module.output)
            end
...
local function trainPaletteChecker(model, loader, opt, epoch)
    ...
    local feval = function(x)
        ...
        for superBatch = 1, opt.transformerSuperBatches do
            ...
            if superBatch == 1 then
                paletteUpdate1:resize(model.paletteValues1.data.module.output:size()):copy(model.paletteValues1.data.module.output)
                paletteUpdate2:resize(model.paletteValues2.data.module.output:size()):copy(model.paletteValues2.data.module.output)
            end
